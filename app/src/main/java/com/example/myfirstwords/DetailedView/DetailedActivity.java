...
//TODO:Replace repeating methods
public class DetailedActivity extends AppCompatActivity {
    ...
    public void populateAnimalsList(int language) {
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            int sound_effectID = getResources().getIdentifier(sound_effects[i], "raw",
                    getPackageName());
    ...
    public void populateAlphabetList(int language) {

        if (language == 2) {
            for (char i = 'A'; i <= 'Z'; i++) items_list.add(new MenuItem(i));
            if (language == 1) {
                for (char i = 'А'; i <= 'Я'; i++) items_list.add(new MenuItem(i));
            }
        }
    }
    ...
    public void addItemtoList(String[] images, String[] names) {
        ...
        for (int i = 0; i < images.length; i++) {
            int imageId = getResources().getIdentifier(images[i], "drawable", getPackageName());
            items_list.add(new MenuItem(imageId, names[i].replaceAll("_", " ")
                    .replaceAll("color", "").trim()));
        }
    }
}